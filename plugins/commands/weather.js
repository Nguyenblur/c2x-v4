module.exports = {
    ...
    async onCall({ api, args, message }) {
      ...
      if (!fs.existsSync('./databases/cache/card/bgweather.jpg')) {
        let getbg = (await axios.get(`https://i.imgur.com/1Rx88Te.jpg`, { responseType: "arraybuffer" })).data;
        fs.writeFileSync("./databases/cache/card/bgweather.jpg", Buffer.from(getbg, "utf-8"));
      }
      
      if (!fs.existsSync("./databases/font/Play-Bold.ttf")) {
        ...
        fs.writeFileSync("./databases/font/Play-Bold.ttf", Buffer.from(getfont, "utf-8"));
      }
      ...
      Canvas.registerFont("./databases/font/Play-Bold.ttf", { family: "Play-Bold" });
      
      const bg = await Canvas.loadImage("./databases/cache/card/bgweather.jpg");
      ...
      for (let item of dataWeather.DailyForecasts.slice(0, 7)) {
      ...
      }, message.threadID, () => fs.unlinkSync(pathSaveImg), message.messageID);
    }
  };
