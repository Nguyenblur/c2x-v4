const fs = require('fs');
const path = './db/data/nsfw/nsfwGroups.json';
  ...
  async onCall({ api, message, args, getText }) {
    ...
    if (command === 'on') {
      if (nsfwGroups.includes(groupID)) {
        api.sendMessage('NSFW Mode đã được bật cho nhóm này!', message.threadID);
      } else {
      ...
      if (!nsfwGroups.includes(groupID)) {
        api.sendMessage('NSFW Mode đã được tắt cho nhóm này!', message.threadID);
      } else {
